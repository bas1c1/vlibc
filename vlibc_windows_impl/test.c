...
vlibc_uint32_t frag_shader_f(vlibc_vec2d frag_pos, vlibc_rgba frag_color, vlibc_shader_data_t *data) {
  ...
  float ambient = 0.06f;
  vlibc_vec3d lightColor = (vlibc_vec3d){VLIBC_ABS(float, vlibc_cos(__vlibc_windows_time * 0.01f)), VLIBC_ABS(float, vlibc_sin(__vlibc_windows_time * 0.01f)), 1};
  vlibc_vec3d lightPos = (vlibc_vec3d){vlibc_sin(__vlibc_windows_time * 0.01f) * 1.5f, vlibc_cos(__vlibc_windows_time * 0.01f) * 1.5f, 2};
...
void display() {
  memset(canvas.pixels, vlibc_rgba_to_hex((vlibc_rgba) {0, 0, 0, 255}), WIDTH * HEIGHT * sizeof(vlibc_uint32_t));
...
#define elementsCount sizeof(vertices) / sizeof(float) / floatsPerElement
  ...
  transformation = vlibc_mat4_mul(vlibc_mat4_rotation(__vlibc_windows_time * VLIBC_DEG2RAD * 0.5f, (vlibc_vec3d){1, 1, 1}), transformation);
...
int main(int argc, char *argv[]) {
  ...
  canvas = vlibc_windows_alloc_canvas((vlibc_vec2d){WIDTH, HEIGHT});

  vlibc_fill(&canvas, (vlibc_rgba){255, 0, 0, 255}, vlibc_nullptr, vlibc_nullptr);
  ...
  vlibc_windows_create_window("test", WIDTH, HEIGHT);
  vlibc_windows_set_canvas(&canvas);
  vlibc_windows_start(display);
