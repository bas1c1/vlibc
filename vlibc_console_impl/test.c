...
vlibc_uint32_t frag_shader_f(vlibc_vec2d frag_pos, vlibc_rgba frag_color, vlibc_shader_data_t *data) {
  ...
  float ambient = 0.06f;
  vlibc_vec3d lightColor = (vlibc_vec3d){VLIBC_ABS(float, vlibc_cos(__vlibc_console_time * 0.01f)), VLIBC_ABS(float, vlibc_sin(__vlibc_console_time * 0.01f)), 1};
  vlibc_vec3d lightPos = (vlibc_vec3d){vlibc_sin(__vlibc_console_time * 0.01f) * 1.5f, vlibc_cos(__vlibc_console_time * 0.01f) * 1.5f, 2};
...
#define elementsCount sizeof(vertices) / sizeof(float) / floatsPerElement
  ...
  transformation = vlibc_mat4_mul(vlibc_mat4_rotation(__vlibc_console_time * VLIBC_DEG2RAD * 0.5f, (vlibc_vec3d){1, 1, 1}), transformation);
...
int main(int argc, char *argv[]) {
  ...
  canvas = vlibc_console_alloc_canvas((vlibc_vec2d){WIDTH, HEIGHT});
  ...
  for(;;) {
    display();
    if (__vlibc_console_time >= (vlibc_uint64_t)-1)
      __vlibc_console_time = 0;
    __vlibc_console_time ++;
  }
}
